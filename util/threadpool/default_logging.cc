...
void LogMessage::GenerateLogMessage() {
  static EnvTime* env_time = prediction::EnvTime::Default();
...
int64 MinLogLevelFromEnv() {
  const char* tf_env_var_val = getenv("PREDICTION_UTIL_CPP_MIN_LOG_LEVEL");
  return LogLevelStrToInt(tf_env_var_val);
}

int64 MinVLogLevelFromEnv() {
  const char* tf_env_var_val = getenv("PREDICTION_UTIL_CPP_MIN_VLOG_LEVEL");
  return LogLevelStrToInt(tf_env_var_val);
}
...
LogMessage::~LogMessage() {
  ...
  if (COMMON_PREDICT_TRUE(severity_ >= min_log_level)) GenerateLogMessage();
}
