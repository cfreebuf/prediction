...
#ifndef COMMON_CORE_PLATFORM_ENV_H_
#define COMMON_CORE_PLATFORM_ENV_H_
...
class Env {
 public:
  ...
  virtual Thread* StartThread(const ThreadOptions& thread_options,
                              const string& name,
                              std::function<void()> fn) COMMON_MUST_USE_RESULT = 0;
#if 0
...
#endif
  COMMON_DISALLOW_COPY_AND_ASSIGN(Env);
...
class Thread {
 ...
 private:
  COMMON_DISALLOW_COPY_AND_ASSIGN(Thread);
};
...
#define REGISTER_FILE_SYSTEM_UNIQ(ctr, env, scheme, factory)   \
  static ::tensorflow::register_file_system::Register<factory> \
      register_ff##ctr COMMON_ATTRIBUTE_UNUSED =                   \
...
#endif  // COMMON_CORE_PLATFORM_ENV_H_
